...
class RemotePlatform {
  ...
  clean (conn) {
    return new Promise((resolve, reject)=>{
      // conn.exec(`screen -ls | grep Detached | cut -d. -f1 | awk '{print $1}' | xargs kill`, (err, stream)=>{
      conn.exec(`rm -rf .lotion`, (err, stream)=>{
        if (err) {
          reject(err)
        }
  ...
  start (conn) {
    let that = this
    return new Promise((resolve, reject)=>{
      let execString = `screen -S platform -d -m node ${that.platformOptions.app} --genesis "${that.platformOptions.genesis}" --keys "${that.platformOptions.keys}" --datarate ${that.platformOptions.datarate} --blocktime ${that.platformOptions.blocktime} ${that.platformOptions.logTendermint?"--logTendermint":""} ${that.platformOptions.peers} -t 46659`
  ...
  async autostart() {
    ...
    let result = await this.start(connection)
    connection.end()
    return result
  }
  ...
  async autoclean() {
    let connection = await this.connect()
    let result = await this.clean(connection)
    connection.end()
    return result
  }
}

module.exports = RemotePlatform
